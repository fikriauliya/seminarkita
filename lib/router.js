...
Router.map(function(){
  ...
  this.route('registrantsListDownload', {
    where: 'server',
    path: '/events/:_id/registrants/download',
    action: function() { 
      data = Registrants.find({eventIds: {$elemMatch: { $in: [this.params._id]}}}, 
        {fields: {name: 1, email: 1, phone: 1}, sort: {name:1, phone:1, email:1}}).fetch();
